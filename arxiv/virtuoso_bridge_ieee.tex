...
This work was motivated by our experience building AMS-IO-Agent~\cite{amsioagent}, an LLM-based agent for I/O ring design that was validated in a 28nm CMOS tape-out. In that project, the need for a stable, remote-capable bridge between the agent and Virtuoso became the central infrastructure challenge. Virtuoso-Bridge is the distilled and generalized result of that effort.
...
\begin{figure}[!t]
\centering
% TODO: replace with hand-drawn architecture diagram
\framebox[\columnwidth]{\parbox{0.9\columnwidth}{\centering\vspace{2.5cm}\textit{Architecture diagram placeholder}\\[6pt]\small Remote Mode: VirtuosoClient $\to$ TCP $\to$ SSH tunnel $\to$ RAMIC daemon $\to$ Virtuoso\\Local Mode: VirtuosoClient $\to$ TCP $\to$ RAMIC daemon $\to$ Virtuoso\vspace{2.5cm}}}
\caption{Decoupled architecture. VirtuosoClient connects to a TCP endpoint without knowing whether it is local or tunneled. SSHClient (optional) creates the tunnel and deploys the daemon.}
...
Virtuoso-Bridge provides SSH remote access, script injection, structured layout and schematic APIs, Spectre simulation integration, multimodal feedback, and agent-native lifecycle management. These capabilities are designed for automated, remote, high-frequency workflows driven by coding agents.
...
The system has been verified in a TSMC 28nm tape-out workflow. The complete version of the harness, from which virtuoso-bridge-lite is distilled, was used in the development of AMS-IO-Agent~\cite{amsioagent}, where an LLM-based agent autonomously completed I/O ring design tasks with outputs directly used in silicon fabrication. That work demonstrated the practical necessity of a stable, remote-capable bridge for agent-driven AMS design.
